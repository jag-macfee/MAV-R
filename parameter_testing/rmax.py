...
# Only r_max changes in this experiment.
DELTA_X = c / N
RMAX_OVER_DX_VALUES = np.array([1.0, 2.0, 4.0, 6.0, 8.0, 12.0, 16.0, 24.0, 32.0])

# Keep the other two WC parameters fixed.  N_max is deliberately large enough
# that r_max remains the active grouping constraint over this sweep.
MIN_LUMPING_DISTANCE_FROM_AF = 1 * c
MAX_VORTICES_TO_LUMP = N
...
PLOT_DIR = Path(__file__).resolve().parent / "plots" / "rmax"
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, lift_errors, marker="o")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, frequency_errors_db, marker="o")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, wagner_errors, marker="o")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, kussner_errors, marker="o")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, impulse_time_fractions, marker="o")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, wagner_time_fractions, marker="o")
ax.axhline(1.0, linestyle="--", linewidth=1.0, label="No-lumping reference")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
fig, ax = plt.subplots()
ax.plot(RMAX_OVER_DX_VALUES, kussner_time_fractions, marker="o")
ax.axhline(1.0, linestyle="--", linewidth=1.0, label="No-lumping reference")
ax.set_xlabel(r"Maximum lumping distance, $r_{max}/\Delta x$")
...
for i, rmax_over_dx in enumerate(RMAX_OVER_DX_VALUES):
    ax.annotate(
        f"{rmax_over_dx:g}",
