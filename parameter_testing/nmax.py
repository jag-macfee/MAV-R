...
# Only N_max changes in this experiment.
MAX_VORTICES_TO_LUMP_VALUES = np.array(
    [2, 3, 4, 6, 8, 10, 12, 14, 16, 18, 20], dtype=int
)

# Keep the other two WC parameters fixed.  r_max is deliberately large enough
# that N_max remains the active grouping constraint over most of this sweep.
MIN_LUMPING_DISTANCE_FROM_AF = 1 * c
MAX_LUMPING_DISTANCE = 0.5 * c
...
PLOT_DIR = Path(__file__).resolve().parent / "plots" / "nmax"
...
for max_vortices_to_lump in MAX_VORTICES_TO_LUMP_VALUES:
    strategy = WCStrategy(
        min_lumping_distance_from_af=MIN_LUMPING_DISTANCE_FROM_AF,
    ...
    print(
        f"N_max = {max_vortices_to_lump:2d} | "
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, lift_errors, marker="o")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, frequency_errors_db, marker="o")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, wagner_errors, marker="o")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, kussner_errors, marker="o")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, impulse_time_fractions, marker="o")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, wagner_time_fractions, marker="o")
ax.axhline(1.0, linestyle="--", linewidth=1.0, label="No-lumping reference")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
fig, ax = plt.subplots()
ax.plot(MAX_VORTICES_TO_LUMP_VALUES, kussner_time_fractions, marker="o")
ax.axhline(1.0, linestyle="--", linewidth=1.0, label="No-lumping reference")
ax.set_xlabel(r"Maximum vortices per lump, $N_{max}$")
...
for i, max_vortices_to_lump in enumerate(MAX_VORTICES_TO_LUMP_VALUES):
    ax.annotate(
        f"{max_vortices_to_lump:d}",
